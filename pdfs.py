...
class dist(ABC):
    ...
    @classmethod
    def __call__(cls, x, *args, loc=0, scale=1, **kwargs):
...
class trunc_dist(dist):
    ...
    @classmethod
    def __call__(cls, x, *args, loc=0, scale=1, xmin=-INF, xmax=INF, **kwargs):
        template = xp.zeros_like(x, dtype=xp.float64)
        x_, xmin_, xmax_ = rescale(x, loc, scale), rescale(xmin, loc, scale), rescale(xmax, loc, scale)

        mask = (x > xmin) & (x < xmax)
        template[mask] = cls._pdf(x_[mask], *args, **kwargs) / scale
        
        # normalize
        cdf_high = cls._cdf(xmax_, *args, **kwargs) if xmax < INF else 1
        cdf_low = cls._cdf(xmin_, *args, **kwargs) if xmin > -INF else 0
        return template / (cdf_high - cdf_low)
    
    @classmethod 
    def logpdf(cls, x, *args, loc=0, scale=1, xmin=-INF, xmax=INF, **kwargs):
        template = xp.full_like(x, -INF, dtype=xp.float64)
        x_, xmin_, xmax_ = rescale(x, loc, scale), rescale(xmin, loc, scale), rescale(xmax, loc, scale)

        mask = (x > xmin) & (x < xmax)
        template[mask] = cls._logpdf(x_[mask], *args, **kwargs) - xp.log(scale)
        
        # normalize
        cdf_high = cls._cdf(xmax_, *args, **kwargs) if xmax < INF else 1
        cdf_low = cls._cdf(xmin_, *args, **kwargs) if xmin > -INF else 0
        return template - xp.log(cdf_high - cdf_low)
...
class smoothed_powerlaw(dist):
    ...
    def _pdf(x, alpha, xmin, xmax, p):
        ...
        # PDF inside support
        x_valid = x[mask]
        vals = (1/C) * xp.power(x_valid, -alpha) * xp.power(1 - xmin / x_valid, p)
    ...
    def _logpdf(x, alpha, xmin, xmax, p):
        ...
        # Main terms
        x_valid = x[mask]
        lnvals = -alpha * xp.log(x_valid) + (p) * xp.log1p(-xmin / x_valid) - lnC
...
class LVK_Plancktaper_powerlaw(dist):
    ...
    @staticmethod
    def _pdf(x, alpha, xmin, xmax, delta):
        # powerlaw
        good = (x > xmin) * (x < xmax) * (xmax > xmin)
        out = xp.zeros_like(x, dtype=xp.float64)
        if not xp.any(good):
            return out
        
        func_unnorm = lambda x: xp.power(x, -alpha) * sigmoid_smooth(x, xmin, delta)

        out[good] = func_unnorm(x[good])

        # compute normalization 
        xx_int = xp.linspace(xmin, xmax, 1000)
        out_int = func_unnorm(xx_int)
        return out / xp.trapz(out_int, xx_int)
    
    @staticmethod
    def _logpdf(x, alpha, xmin, xmax, delta):
        # powerlaw
        good = (x > xmin) * (x < xmax) * (xmax > xmin)
        out = xp.full_like(x, -INF, dtype=xp.float64)
        if not xp.any(good):
        ...
        xgood = x[good]
        out[good] = -alpha * xp.log(xgood) + log_sigmoid_smooth(xgood, xmin, delta)

        # compute normalization 
        xx_int = xp.linspace(xmin, xmax, 1000)
        out_int = xp.power(xx_int, -alpha) * log_sigmoid_smooth(xx_int, xmin, delta)
        return out - xp.log(xp.trapz(out_int, xx_int))

class BaseRate:

    def __init__(self, cosmo=Planck15):
        self.cosmo = cosmo
        self._z_arr = xp.concatenate([xp.linspace(EPS, 5, 1000, endpoint=False), xp.linspace(5, 10, 100)])
        self._dV_dz_arr = 4*xp.pi * self.cosmo.differential_comoving_volume(self._z_arr).value # in Mpc^3
    ...
    @abstractmethod
    def log_psi(z, *args, **kwargs):
        """
        Log rate model, normalized such that log_psi(0)=0.
        """
        raise NotImplementedError

    def __call__(self, z, R0, *args, **kwargs):
        return R0 * self.psi(z, *args, **kwargs) / (1.+ z) * self.dV_dz(z)

    def lograte(self, z, R0, *args, **kwargs):
        return xp.log(R0) + self.log_psi(z, *args, **kwargs) - xp.log1p(z) + self.log_dV_dz(z)
