...
if __name__ == "__main__":
    ...
    if args.in_path is None:
        parser.error(
            "--in_path is required (or set 'gwtc_injs_in_path' in ~/.rjpop_config.json)"
        )
    if args.out_path is None:
        parser.error(
            "--out_path is required (or set 'out_path' in ~/.rjpop_config.json)"
        )
    ...
    update_config("injs_path", outfile)
    ...
    with h5py.File(args.in_path, "r") as f_:
        ...
        sampling_pdf = np.exp(
            f[
                "lnpdraw_mass1_source_mass2_source_redshift_spin1_magnitude_spin1_polar_angle_spin1_azimuthal_angle_spin2_magnitude_spin2_polar_angle_spin2_azimuthal_angle"
            ][()]
        )
        sampling_pdf *= m1  # m1, m2 -> m1, q
        sampling_pdf /= (
            (1 / (4 * np.pi)) ** 2 * np.sin(theta1) * np.sin(theta2)
        )  # uniform prior on spins
        sampling_pdf *= chi_effective_prior_from_isotropic_spins(
            chi_eff=chi_eff, q=q, aMax=1.0
        )
        ...
        far_min = np.min(
            [f[name][()] for name in f.dtype.names if name.endswith("_far")], axis=0
        )
        ...
        mask = ((rho_opt >= args.rho_thr) & ~np.isfinite(far_min)) | (
            far_min <= args.far_thr
        )
