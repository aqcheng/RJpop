...
rj_moves = [] # one for each branch
if ngauss_max > ngauss_min:
    prior_move_gauss = DistributionGenerateRJ( # add leaf by drawing from the prior
        priors, nleaves_min=dict_nleaves_min, nleaves_max=dict_nleaves_max,
        gibbs_sampling_setup="gauss" # this is to ensure only changing one branch at a time
    )
