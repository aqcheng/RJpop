##########################################################################
# This is a slightly modified version of the implementation of the chi_eff 
# Jacobian in `effective-spin-priors`; this code is lifted directly from
...
def chi_effective_prior_from_aligned_spins(chi_eff, q, aMax=0.8):
    ...
    chi_eff: Chi_effective value or values at which we wish to compute prior
    ...
    caseA = (chi_eff>aMax*(1.-q)/(1.+q))*(chi_eff<=aMax)
    caseB = (chi_eff<-aMax*(1.-q)/(1.+q))*(chi_eff>=-aMax)
    caseC = (chi_eff>=-aMax*(1.-q)/(1.+q))*(chi_eff<=aMax*(1.-q)/(1.+q))
...
# I MODIFIED THIS TO ALLOW FOR VECTORIZED Q
def chi_effective_prior_from_isotropic_spins(chi_eff, q, aMax = 0.998):
    ...
    # Ensure that `chi_eff` is an array and take absolute value
    chi_eff = np.asarray(np.abs(chi_eff))

    # Set up various piecewise cases
    pdfs = np.ones_like(chi_eff,dtype=complex)*(-1.)
    caseZ = (chi_eff==0)
    caseA = (chi_eff>0)*(chi_eff<aMax*(1.-q)/(1.+q))*(chi_eff<q*aMax/(1.+q))
    caseB = (chi_eff<aMax*(1.-q)/(1.+q))*(chi_eff>q*aMax/(1.+q))
    caseC = (chi_eff>aMax*(1.-q)/(1.+q))*(chi_eff<q*aMax/(1.+q))
    caseD = (chi_eff>aMax*(1.-q)/(1.+q))*(chi_eff<aMax/(1.+q))*(chi_eff>=q*aMax/(1.+q))
    caseE = (chi_eff>aMax*(1.-q)/(1.+q))*(chi_eff>aMax/(1.+q))*(chi_eff<aMax)
    caseF = (chi_eff>=aMax)
    ...
    if np.any(pdfs==-1):
        boundary = (pdfs==-1)
        pdfs[boundary] = 0.5*(chi_effective_prior_from_isotropic_spins(chi_eff[boundary]+1e-6, q[boundary], aMax)\
                        + chi_effective_prior_from_isotropic_spins(chi_eff[boundary]-1e-6, q[boundary], aMax))
